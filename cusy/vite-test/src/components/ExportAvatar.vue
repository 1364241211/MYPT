...
const downloadZipFile = async (index: number) => {
  ...
  const zipinfoType: string = avatarFileInfo.value.at(index)?.zipinfo_type!;
  const typeTeacherUrl = "exportTeacherAvatar";
  const typeUserUrl = "exportAvatar";
  let finalUrl = typeUserUrl;
  if (zipinfoType == "1") {
    finalUrl = typeTeacherUrl;
  }
  ...
  for (let i = 0; i < SLICE_COUNT; i++) {
    ...
    if (i == SLICE_COUNT - 1)
      taskQueen.push(
        serq(`/${finalUrl}?uuid=${uuid}`, i * Q, fileLength, index, i)
      );
    else
      taskQueen.push(
        serq(`${finalUrl}?uuid=${uuid}`, i * Q, (i + 1) * Q - 1, index, i)
      );
...
// 发送导出图片请求
const exportAvatar = async (type: number) => {
  const typeTeacherUrl = "exportTeacherAvatar";
  const typeUserUrl = "exportAvatar";
  let finalUrl = typeUserUrl;
  if (type == 1) {
...
const convToType = (row: any, column: any, cellValue: any, index: any) => {
  cellValue = cellValue == "1" ? "教师" : "学生";
  return cellValue;
};
