    ...
    <el-config-provider :locale="locale">
      ...
      >
        ...
        <el-form-item label="教师名称" prop="teacher_name">
          <el-input v-model="forms.teacher_name"></el-input>
        </el-form-item>
        <el-form-item label="教师工号" prop="customer_id">
          <el-input v-model="forms.teacher_id"></el-input>
        </el-form-item>
        <el-form-item label="教师职称" prop="parent_phone">
          <el-input v-model="forms.teacher_department"></el-input>
        </el-form-item>
        <el-form-item label="联系方式" prop="parent_phone">
          <el-input v-model="forms.teacher_phone"></el-input>
        </el-form-item>
        ...
        <el-form-item label="教师登记时间">
          <el-date-picker
            v-model="forms.teacher_logtime"
            type="datetime"
            placeholder="请选择时间"
          />
        </el-form-item>
...
const props = defineProps<{ itemProps: teacher }>();

const formInstance = ref<FormInstance>();

const forms = reactive({
  id: props.itemProps.id,
  teacher_name: props.itemProps.teacher_name,
  teacher_id: props.itemProps.teacher_id,
  teacher_department: props.itemProps.teacher_department,
  teacher_photo: props.itemProps.teacher_photo,
  teacher_phone: props.itemProps.teacher_phone,
  teacher_logtime: new Date(props.itemProps.teacher_logtime),
});

const vaildName = (rule: any, value: any, callback: any) => {
  value = forms.teacher_name;
  if (value === "") {
...
const vaildId: (rule: any, value: any, callback: any) => void = async (
  ...
  if (value == props.itemProps.teacher_id) {
    callback();
  } else if (value === "") {
    callback(new Error("请输入工号！"));
  } else {
    const { res, error } = await useRequest(
      `/teacherGeneralApi?teacher_id=${value}`
    );
    if (res.value) {
...
const updateCustomerPhoto = ref<UploadUserFile[]>([
  {
    name: props.itemProps.teacher_photo,
    url: `${import.meta.env.VITE_APP_STATIC_URL}/static/avatar/${
      props.itemProps.teacher_photo
    }`.trim(),
  },
]);
// 判断更改前是否是同一张图片
  ...
  if (!isSame.value) {
    return service
      .post(
        "/uploadTeacherAvatar",
        JSON.stringify({
          avatarName: `${forms.teacher_id}.${avatarType.value}`,
...
const updateCustomer = () => {
  formInstance.value?.validate((value: any) => {
    if (value) {
      ...
      let formData = {
        teacher_id: forms.teacher_id,
        teacher_name: forms.teacher_name,
        teacher_department: forms.teacher_department,
        teacher_photo: `${forms.teacher_id}.${avatarType.value}`,
        teacher_phone: forms.teacher_phone,
      };
      const UpdateInfo = service
        .put(
          `/teachers?teacher_id=${props.itemProps.teacher_id}`,
