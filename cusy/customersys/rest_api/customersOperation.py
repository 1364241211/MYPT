...
class customersOp():
    def __init__(self, customers_photo):
        ...
        self.zipFilePath = os.path.join(
            settings.STATICFILES_DIRS[0], "zipFile")
    ...
    def readAvatarZip(self, zipFileName: str, filters: list = None, zipInfoType: int = 0):
        ...
        tempZipPath = os.path.join(
            self.zipFilePath, "{0}.zip".format(tempName))
        ...
        if zipInfoList.count() == 3:
            ...
            if newZipInfo.download == 0:
                global_thread_pool.destroy_task(
                    os.path.splitext(newZipInfo.zip_name)[0])
            with self.sync_lock:
                tempRemoveFilePath = os.path.join(
                    self.zipFilePath, newZipInfo.zip_name)
        ...
        newZipInfo.zipinfo_type = zipInfoType
        newZipInfo.save()
        print(filters)
        if filters:
